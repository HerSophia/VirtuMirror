...
const appPrompts = computed(() => systemPrompts.value.filter((p) => p.systemPromptScope === 'app'))
...
const handleCreate = () => {
  ...
  SystemPromptService.create({
    ...
    applicableScenes: formData.value.applicableScenes
      ? formData.value.applicableScenes
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean)
      : undefined,
    excludeScenes: formData.value.excludeScenes
      ? formData.value.excludeScenes
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean)
      : undefined,
    ...
    <div class="content-scroll">
      ...
      <section v-if="globalPrompts.length > 0" class="section">
        ...
        <div class="prompt-list">
          <div v-for="prompt in globalPrompts" :key="prompt.id" class="prompt-item">
            <div class="prompt-main" @click="openEditDialog(prompt)">
              ...
              <p class="prompt-preview">
                {{ prompt.template.slice(0, 100) }}{{ prompt.template.length > 100 ? '...' : '' }}
              </p>
            ...
            <div class="prompt-actions">
              ...
              <button class="action-btn delete-btn" @click.stop="handleDelete(prompt)" title="删除">
        ...
        <div class="prompt-list">
          <div v-for="prompt in prompts" :key="prompt.id" class="prompt-item">
            <div class="prompt-main" @click="openEditDialog(prompt)">
              <div class="prompt-header">
                <span class="prompt-name">{{ prompt.name }}</span>
                <span v-if="prompt.systemPromptMode === 'override'" class="override-badge"
                  >覆盖</span
                >
                <span v-if="!prompt.enabled" class="disabled-badge">已禁用</span>
              </div>
            ...
            <div class="prompt-actions">
              ...
              <button class="action-btn delete-btn" @click.stop="handleDelete(prompt)">
                <i class="fas fa-trash"></i>
              </button>
            </div>
          </div>
    ...
    <div v-if="showEditDialog" class="dialog-overlay" @click.self="showEditDialog = false">
      <div class="dialog">
        ...
        <div class="dialog-content">
          ...
          <div class="form-group">
            <label class="form-label">提示词内容 *</label>
            <textarea v-model="formData.content" class="form-textarea" rows="6"></textarea>
          </div>
