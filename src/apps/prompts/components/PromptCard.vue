...
<template>
  <div class="prompt-card" :class="{ disabled: !prompt.enabled }" @click="handleClick">
    <!-- 头部信息 -->
    ...
    <div class="card-tags">
      ...
      <span v-if="prompt.priority > 0" class="tag"> 优先级: {{ prompt.priority }} </span>
    </div>
    ...
    <div class="card-preview">
      <details>
        ...
        <pre class="preview-content"
          >{{ prompt.template.substring(0, 300)
          }}{{ prompt.template.length > 300 ? '...' : '' }}</pre
        >
      </details>
    </div>
    ...
    <div class="card-actions">
      <button class="action-btn" @click="handleToggle" :title="prompt.enabled ? '禁用' : '启用'">
        <i :class="prompt.enabled ? 'fas fa-toggle-on' : 'fas fa-toggle-off'"></i>
      </button>

      <button v-if="permission.canEdit" class="action-btn" @click="handleEdit" title="编辑">
...
<style scoped>
.prompt-card {
  @apply cursor-pointer rounded-xl p-4;
...
.tag {
  @apply flex items-center gap-1 rounded px-2 py-0.5 text-xs;
  background: var(--color-surface-variant);
  color: var(--color-text-secondary);
}
...
.preview-toggle {
  @apply flex cursor-pointer items-center gap-1.5 text-sm;
  color: var(--color-primary);
}
...
.preview-content {
  @apply mt-2 overflow-x-auto whitespace-pre-wrap rounded-lg p-3 text-xs;
  @apply font-mono;
  background: var(--color-surface-variant);
  color: var(--color-text);
  max-height: 150px;
...
/* 操作按钮 */
.card-actions {
  @apply flex items-center gap-1 border-t pt-3;
...
/* 底部 */
.card-footer {
  @apply mt-2 flex items-center justify-between pt-2 text-xs;
  color: var(--color-text-secondary);
