...
async function loadLikes() {
  ...
  try {
    const likedIds = userActionStore.getLikedPostIds();
...
<template>
  <div class="h-full flex flex-col bg-gray-50">
    ...
    <header class="flex-none bg-white border-b border-gray-100 px-4 py-3 flex items-center">
      ...
      <h1 class="flex-1 text-center text-base font-medium">我的赞</h1>
      <div class="w-8"></div>
    </header>
    ...
    <main class="flex-1 overflow-y-auto">
      ...
      <div v-else-if="isEmpty" class="h-full flex flex-col items-center justify-center p-6">
        ...
        <h3 class="text-base font-medium text-gray-600 mb-2">暂无点赞</h3>
        <p class="text-sm text-gray-400 text-center">
          浏览微博时点击赞按钮，喜欢的内容会出现在这里
        </p>
      </div>
