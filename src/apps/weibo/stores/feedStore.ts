...
import type { DisplayPost, PostAuthor, PrimaryContentType, UniversalPost } from '@/types/social'
import { getWeiboPrimaryType } from '@/types/social'
import { defineStore } from 'pinia'
import { v4 as uuidv4 } from 'uuid'
import { computed, ref } from 'vue'
import {
  messageGridItems as mockGrid,
  messageListItems as mockMessages,
...
export const useFeedStore = defineStore('weiboFeed', () => {
  ...
  async function refreshFeed() {
    ...
    const storedPosts = await db.socialPosts
      .where('platformId')
      .equals('weibo')
      .filter((p) => (!namespace || p.namespace === namespace) && sourceFilter(p))
      .reverse()
      .sortBy('timestamp')
  ...
  async function getPostsByAuthor(authorId: string): Promise<DisplayPost[]> {
    ...
    const allPosts = await db.socialPosts.where('authorId').equals(authorId).toArray()
  ...
  async function getPostCountByAuthor(authorId: string): Promise<number> {
    ...
    return allPosts.filter(
      (p) => p.platformId === 'weibo' && (!namespace || p.namespace === namespace)
    ).length
  }
  ...
  async function getUserInfo(authorId: string): Promise<WeiboUser> {
    ...
    if (platformAccount) {
      ...
      return {
        ...
        avatar:
          platformAccount.avatarOverride ||
          entity?.avatar ||
   ...
   */
  function setAutoGenerateConfig(
    config: Partial<{ onEmptyFeed: boolean; onFewComments: boolean }>
  ) {
    const settingsStore = useSettingsStore()
   ...
   */
  async function ensureCommenterAccount(nickname: string, persona?: string): Promise<string> {
    // 尝试查找已存在的账号（通过昵称）
  ...
  async function getPostStats(): Promise<{ posts: number; comments: number }> {
    ...
    return {
      ...
      comments: allComments.filter(
        (c) => c.platformId === 'weibo' && (!namespace || c.namespace === namespace)
      ).length,
    }
  }
