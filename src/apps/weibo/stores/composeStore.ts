...
import type { WeiboDraft, ComposePostData } from '../types';
...
import type { UniversalPost, PrimaryContentType, MediaAsset, PostPayload, ContentFlags } from '@/types/social';
...
export const useComposeStore = defineStore('weiboCompose', () => {
   ...
   * 保存草稿（新建或更新）
   */
  async function saveDraft(draftData: Omit<WeiboDraft, 'id' | 'createdAt' | 'updatedAt'>, draftId?: string): Promise<WeiboDraft> {
    const now = Date.now();
  ...
  async function expandVideoDescription(videoDescription: string): Promise<{
    ...
    try {
      ...
      return {
        ...
        suggestedDuration: parsed.suggestedDuration ? parseInt(parsed.suggestedDuration) : undefined,
      };
    } catch (error: any) {
  ...
  async function publishPost(
    ...
    try {
      ...
      if (!currentAccount) {
        ...
        currentAccount = await accountStore.createPlatformAccount(
          player.id,
          'weibo',
          {
      ...
      // 1. 确定主类型
      const primaryType: PrimaryContentType = postData.type === 'poll' 
        ? 'poll' 
        : postData.type === 'video' 
          ? 'video' 
          : (postData.images.length >= 4 ? 'gallery' : 'text');
