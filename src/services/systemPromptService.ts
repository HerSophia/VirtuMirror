...
import type { PromptTemplate, SystemPromptScope, SystemPromptMode, PromptVariable } from '@/types/prompts';
...
export class SystemPromptService {
  ...
  static assemble(options: AssembleOptions = {}): AssembleResult {
    ...
    // 1. 收集全局系统提示词
    const globalPrompts = this.getGlobalSystemPrompts()
      .filter(p => this.isApplicableToScene(p, scene));
    ...
    if (appId) {
      appPrompts = this.getAppSystemPrompts(appId)
        .filter(p => this.isApplicableToScene(p, scene));
  ...
  static create(definition: SystemPromptDefinition): PromptTemplate {
    const { name, description, content, scope, appId, mode, applicableScenes, excludeScenes, priority } = definition;
  ...
  static update(id: string, updates: Partial<{
    ...
    if (updates.applicableScenes !== undefined) promptUpdates.applicableScenes = updates.applicableScenes;
  ...
  static preview(options: AssembleOptions): {
    ...
    // 分解每个来源
    const globalPrompts = this.getGlobalSystemPrompts()
      .filter(p => this.isApplicableToScene(p, options.scene));
    ...
    if (options.appId) {
      const appPrompts = this.getAppSystemPrompts(options.appId)
        .filter(p => this.isApplicableToScene(p, options.scene));
